...
/* the forward kinematics take joint values and determine world coordinates,
   ...
   passed in. */
extern int kinematicsForward2( double * joint,
                      PmRotationMatrix* Ehom_M,PmCartesian* Ehom_P,
                       KINEMATICS_FORWARD_FLAGS * fflags,
                      KINEMATICS_INVERSE_FLAGS * iflags);


extern int kinematicsForward( double *joint,
			      RobotPose* hom, PmRotationMatrix *Rtmp,
			     KINEMATICS_FORWARD_FLAGS * fflags,
			     KINEMATICS_INVERSE_FLAGS * iflags);
...
/* the inverse kinematics take world coordinates and determine joint values,
   ...
   passed in. */
extern int kinematicsInverse( RobotPose * robotpose,PmHomogeneous* hom,
			     double *joint, double *old_joint,
			    int  fflags);
...
/* the home kinematics function sets all its arguments to their proper
   ...
   should be used.
*/
extern int kinematicsHome(PmHomogeneous * hom, 
			  double *joint,
			  KINEMATICS_FORWARD_FLAGS * fflags,
			  KINEMATICS_INVERSE_FLAGS * iflags);
