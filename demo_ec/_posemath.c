...
int pmMatRev(PmRotationMatrix m, PmRotationMatrix * mout)
{
    /* inverse of a rotation matrix is the transpose */
double deltm;
	deltm =m.x.x*m.y.y*m.z.z+m.y.x*m.z.y*m.x.z+m.x.y*m.y.z*m.z.x
		-m.z.x*m.y.y*m.x.z-m.y.x*m.x.y*m.z.z-m.x.x*m.z.y*m.y.z;
	if(fabs(deltm) < RS_FUZZ){
		 return -1;
		}else{
	    mout->x.x = (m.y.y*m.z.z-m.z.y*m.y.z)/deltm;
	    mout->x.y = (m.z.y*m.x.z-m.x.y*m.z.z)/deltm;
	    mout->x.z = (m.x.y*m.y.z-m.y.y*m.x.z)/deltm;

	    mout->y.x = (m.z.x*m.y.z-m.y.x*m.z.z)/deltm;
	    mout->y.y = (m.x.x*m.z.z-m.z.x*m.x.z)/deltm;
	    mout->y.z = (m.y.x*m.x.z-m.x.x*m.y.z)/deltm;

	    mout->z.x = (m.y.x*m.z.y-m.z.x*m.y.y)/deltm;
	    mout->z.y = (m.z.x*m.x.y-m.x.x*m.z.y)/deltm;
	    mout->z.z = (m.x.x*m.y.y-m.y.x*m.x.y)/deltm;
...
int pmMatMatAdd(PmRotationMatrix m1, PmRotationMatrix m2,
    PmRotationMatrix * mout)
{
    mout->x.x = m1.x.x +m2.x.x ;
    mout->x.y = m1.x.y + m2.x.y;
    mout->x.z = m1.x.z + m2.x.z;

    mout->y.x =  m1.y.x + m2.y.x;
    mout->y.y =  m1.y.y + m2.y.y ;
    mout->y.z =  m1.y.z + m2.y.z;

    mout->z.x =  m1.z.x + m2.z.x ;
    mout->z.y =  m1.z.y + m2.z.y ;
    mout->z.z =  m1.z.z + m2.z.z;

    return pmErrno = 0;
}
int pmMatScale(PmRotationMatrix m1, double d,
    PmRotationMatrix * mout)
{
    mout->x.x = m1.x.x *d ;
    mout->x.y = m1.x.y *d;
...
/* line functions */
int pmJointsInit(PmJLine * jointsL,
                 PmJoint start_joint,PmJoint end_joint,int joint){
 ...
 double jmag=0.0;
  ...
  if (0 == jointsL)
    {
#ifdef PM_PRINT_ERROR
      pmPrintError("error: pmJointInit pointer is null\n");
#endif
      return pmErrno = PM_ERR;
    }
#endif
        jointsL->start = start_joint;
        jointsL->end = end_joint;
       switch( joint){
	case 0:
	jmag=fabs(end_joint.j0 - start_joint.j0);
	break;
	case 1:
	jmag=fabs(end_joint.j1 - start_joint.j1);
	break;
	case 2:
	jmag=fabs(end_joint.j2 - start_joint.j2);
	break;
	case 3:
	jmag=fabs(end_joint.j3 - start_joint.j3);
	break;
	case 4:
	jmag=fabs(end_joint.j4 - start_joint.j4);
	break;
	case 5:
	jmag=fabs(end_joint.j5 - start_joint.j5);
	break;
	case 6:
	jmag=fabs(end_joint.j6 - start_joint.j6);
	break;
	case 7:
	jmag=fabs(end_joint.j7 - start_joint.j7);
	break;

	default:
	break;
	   }
	jointsL->jmag = (fabs(jmag)>Q_FUZZ) ? jmag: 0.0;
	jointsL->jmag_zero = (jointsL->jmag <= Q_FUZZ);

  return pmErrno = 0;
}


int pmJointsPoint(PmJLine * jointsL, double len, PmJoint* point){

 if (jointsL->jmag_zero) {
	point->j0 = jointsL->end.j0;
	point->j1 = jointsL->end.j1;
	point->j2 = jointsL->end.j2;
	point->j3 = jointsL->end.j3;
	point->j4 = jointsL->end.j4;
	point->j5 = jointsL->end.j5;
	point->j6 = jointsL->end.j6;
	point->j7 = jointsL->end.j7;
	
    } else {
	
	point->j0=(jointsL->end.j0-jointsL->start.j0) *len/jointsL->jmag;
	point->j1=(jointsL->end.j1-jointsL->start.j1) *len/jointsL->jmag;
	point->j2=(jointsL->end.j2-jointsL->start.j2) *len/jointsL->jmag;
	point->j3=(jointsL->end.j3-jointsL->start.j3) *len/jointsL->jmag;
	point->j4=(jointsL->end.j4-jointsL->start.j4) *len/jointsL->jmag;
	point->j5=(jointsL->end.j5-jointsL->start.j5) *len/jointsL->jmag;
	point->j6=(jointsL->end.j6-jointsL->start.j6) *len/jointsL->jmag;
	point->j7=(jointsL->end.j7-jointsL->start.j7) *len/jointsL->jmag;

	point->j0=(fabs(jointsL->start.j0+point->j0)>Q_FUZZ)?  (jointsL->start.j0+point->j0):0.0;
	point->j1=(fabs(jointsL->start.j1+point->j1)>Q_FUZZ)?  (jointsL->start.j1+point->j1):0.0;
	point->j2=(fabs(jointsL->start.j2+point->j2)>Q_FUZZ)?  (jointsL->start.j2+point->j2):0.0;
	point->j3=(fabs(jointsL->start.j3+point->j3)>Q_FUZZ)?  (jointsL->start.j3+point->j3):0.0;
	point->j4=(fabs(jointsL->start.j4+point->j4)>Q_FUZZ)?  (jointsL->start.j4+point->j4):0.0;
	point->j5=(fabs(jointsL->start.j5+point->j5)>Q_FUZZ)?  (jointsL->start.j5+point->j5):0.0;
	point->j6=(fabs(jointsL->start.j6+point->j6)>Q_FUZZ)?  (jointsL->start.j6+point->j6):0.0;
	point->j7=(fabs(jointsL->start.j7+point->j7)>Q_FUZZ)?  (jointsL->start.j7+point->j7):0.0;
...
int pm3DCircleInit(Pm3DCircle * circle3D,
                 PmPose start,PmPose mid, PmPose end,
                 PmCartesian center, PmCartesian normal,int turn)
{
  double dot;
  PmCartesian v1,v2,v3;
  double d1,d2,d3;
  ...
  if (0 == circle3D)
    {
#ifdef PM_PRINT_ERROR
      pmPrintError("error: pm3DCircleInit cirle pointer is null\n");
#endif
      return pmErrno = PM_ERR;
    }
#endif
...
#ifdef PM_PRINT_ERROR
      pmPrintError("error: pm3DCircleInit normal vector is 0\n");
#endif
      return -1;
    }

  circle3D->center = center;
  pmCartUnit(normal, &circle3D->normal);

  /* radius */
  pmCartCartDisp(start.tran, circle3D->center, &circle3D->radius);

  if(circle3D->radius < DOUBLE_FUZZ)
  {
#ifdef PM_PRINT_ERROR
	pmPrintError("can not init circle, the circle3D radius is zero.\n");
#endif
  	return -1;
  }
  ...
  /* angle */
  pmCartCartSub(start.tran, center, & circle3D->rStart);
  pmCartCartSub(mid.tran, center, & circle3D->rMid);
  pmCartCartSub(end.tran, center, & circle3D->rEnd);

  pmCartCartDot(circle3D->rStart,circle3D->rEnd, &dot);
  dot = dot / (circle3D->radius * circle3D->radius);
  if (dot > 1.0)
    {
      circle3D->angle = 0.0;
    ...
    {
      circle3D->angle = PM_PI;
      
    }
  else
    {
      circle3D->angle = acos(dot);
    }
 
  pmCartCartCross(circle3D->rStart, circle3D->rEnd, &v1);
  pmCartCartCross(circle3D->rStart, circle3D->rMid, &v2);
  pmCartCartCross(circle3D->rMid, circle3D->rEnd, &v3);

  pmCartCartDot(v1, v2, &d1);
  pmCartCartDot(v1, v3, &d2);
  pmCartCartDot(v2, v3, &d3);

 // circle3D->turn = turn;
  if ((((d1*d3> -Q_FUZZ)&&(d1*d2< -Q_FUZZ))||((d1*d3< -Q_FUZZ )&&(d1*d2> -Q_FUZZ)))||((d1*d2<-Q_FUZZ)&&(d1*d3<-Q_FUZZ)))
    {
      circle3D->angle = PM_2_PI - circle3D->angle;
    }

circle3D->tmag = pmSqrt(pmSq(circle3D->angle * circle3D->radius));

circle3D->tmag_zero = (circle3D->tmag <= CART_FUZZ);

 // if (circle->angle == 0.0)
/*
  if (fabs(circle3D->angle) < 1.0e-6) 
  {
     circle3D->angle = PM_2_PI; 
  }
*/
  return pmErrno = 0;
}

int pm3DCirclePoint(Pm3DCircle * circle3D, double angle, PmPose * point)
{
  double semiAngle;
  ...
  if (0 == circle3D ||
    ...
    {
#ifdef PM_PRINT_ERROR
      pmPrintError("error: pm3DCirclePoint circle or point pointer is null\n");
#endif
      return pmErrno = PM_ERR;
    }
#endif
  ...
  if (circle3D->angle == 0.0)
    {
#ifdef PM_PRINT_ERROR
      pmPrintError("error: pm3DCirclePoint angle is zero\n");
#endif
      return pmErrno = PM_DIV_ERR;
    }
	semiAngle = angle / 2.0;
	rotation.s = cos(semiAngle);
	rotation.x = circle3D->normal.x * sin(semiAngle);
	rotation.y = circle3D->normal.y * sin(semiAngle);
	rotation.z = circle3D->normal.z * sin(semiAngle);
	pmQuatNorm(rotation, & rotation);
/*
	if(-1 == circle3D->turn)
	{
		pmQuatInv(rotation, & rotation);
	}

*/	
       pmQuatCartMult(rotation, circle3D->rStart, &point->tran); 
      /* add to center vector for final result */
      pmCartCartAdd(circle3D->center, point->tran, &point->tran);
